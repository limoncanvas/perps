...
export class RiskManager {
  ...
  async calculateRiskMetrics(): Promise<RiskMetrics> {
    try {
      ...
      const usedMargin = parseFloat(accountState.marginSummary.totalMarginUsed || '0');
      const totalUnrealizedPnl = parseFloat(accountState.marginSummary.totalNtlPos || '0');
      ...
      const currentDrawdown = (this.maxEquitySeen - totalEquity) / this.maxEquitySeen;
