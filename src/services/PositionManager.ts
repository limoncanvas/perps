  ...
  async executeSignal(signal: TradeSignal): Promise<void> {
    try {
      ...
      if (signal.action === 'CLOSE') {
        ...
        const pnl = parseFloat(currentPosition.unrealizedPnl || '0');
        
        await this.client.closePosition(signal.coin);
        
        // Record trade with PnL
        if (this.performanceTracker) {
          this.performanceTracker.recordTrade(signal.coin, 'CLOSE', Math.abs(parseFloat(currentPosition.szi)), marketData.price, pnl);
        }
        return;
      }
      ...
      // Record trade
      if (this.performanceTracker) {
        this.performanceTracker.recordTrade(signal.coin, signal.action, signal.size, limitPrice);
      }
    ...
    try {
      ...
      for (const position of positions) {
        ...
        if (pnlPercent <= -this.config.risk.stopLossPercent) {
          ...
          // Record trade with PnL
          if (this.performanceTracker) {
            this.performanceTracker.recordTrade(position.coin, 'CLOSE', Math.abs(parseFloat(position.szi)), currentPrice, pnl);
          }
          continue;
        }
