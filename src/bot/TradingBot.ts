...
export class TradingBot {
  ...
  private performanceSummaryInterval: NodeJS.Timeout | null = null;
  ...
  constructor(
    ...
    this.performanceTracker = new PerformanceTracker(this.client);
    this.positionManager = new PositionManager(this.client, config, this.performanceTracker);
  ...
  async start(updateIntervalMs: number = 60000): Promise<void> {
    ...
    // Set up periodic performance summary (every 30 minutes)
    this.performanceSummaryInterval = setInterval(async () => {
      if (this.isRunning) {
        await this.performanceTracker.logPerformanceSummary();
      }
    }, 30 * 60 * 1000);
  ...
  async stop(): Promise<void> {
    ...
    if (this.performanceSummaryInterval) {
      ...
      this.performanceSummaryInterval = null;
    }
  ...
  private async runTradingCycle(): Promise<void> {
    try {
      ...
      const metrics = await this.riskManager.calculateRiskMetrics();
